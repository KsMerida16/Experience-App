...
class TransactionsView extends ConsumerStatefulWidget {
  const TransactionsView({super.key, this.saleId});
...
class _TransactionsViewState extends ConsumerState<TransactionsView> {
    ...
    return Scaffold(
      ...
      body: txAsync.when(
        ...
        data: (transactions) {
          ...
          // de la venta indicada por saleId.
          if (widget.saleId != null && !_detailOpened) {
            WidgetsBinding.instance.addPostFrameCallback((_) {
              _openSaleFromNotification();
            });
          }
  ...
  Future<void> _openSaleFromNotification() async {
    if (_detailOpened) {
      return;
    }
    ...
    debugPrint('Abriendo venta desde notificación');
    ...
    try {
      final sale = await ref.read(saleByIdProvider(saleId).future);
      ...
      if (sale == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'No se encontró la venta asociada a la notificación.',
            ),
          ),
        );
      ...
      debugPrint('Venta encontrada: ${sale.id}');

      _showTransactionDetail(context, sale);
    } catch (e) {
      debugPrint('Error al obtener la venta $saleId: $e');
